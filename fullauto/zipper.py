...
def get_paths_from_config():
    ...
    try:
        base_dir = config['ZIPPER']['base_dir']
        zipped_dir = config['ZIPPER']['zipped_dir']
        print(f"[INFO] Base directory (from config): {base_dir}")
        print(f"[INFO] Zipped directory (from config): {zipped_dir}")
...
def main():
    print_banner()
    setup_logging()
    base_dir, zipped_dir = get_paths_from_config()
