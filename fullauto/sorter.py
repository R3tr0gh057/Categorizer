...
def get_paths_from_config():
    ...
    try:
        source_dir = config['SORTER']['source_dir']
        destination_dir = config['SORTER']['destination_dir']
        print(f"[INFO] Source directory (from config): {source_dir}")
        print(f"[INFO] Destination directory (from config): {destination_dir}")
        return source_dir, destination_dir
    except Exception as e:
        print(f"[ERROR] Could not read paths from config.ini: {e}")
        logging.error(f"Could not read paths from config.ini: {e}")
        exit(1)
...
def main():
    print_banner()
    setup_logging()
    source_dir, destination_dir = get_paths_from_config()
